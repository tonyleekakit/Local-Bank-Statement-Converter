    ...
    extraction: {
        ...
        headerSkipTolerance: 5,
            ...
            details: {
                ...
                // 匯豐銀行：Details 欄位範圍容差（因為 X 座標可能略有偏差）
                rangeTolerance: 5 // 允許 ±5 像素的偏差
            },
            ...
            date: {
                ...
                patterns: [
                    ...
                    /\d{1,2}[\-\s]+\w{3}/, // 6-Dec, 8-Dec (匯豐銀行格式)
                    /\d{1,2}[\-\s]+\w{3}[\-\s]+\d{2,4}/ // 6-Dec-24
                ],
                // 備用方案：如果沒找到標準日期格式
...
// 註冊到全局配置對象
if (typeof BANK_TEMPLATE_CONFIGS !== 'undefined') {
    BANK_TEMPLATE_CONFIGS.hsbc = HSBC_CONFIG;
} else {
    console.error('BANK_TEMPLATE_CONFIGS 未定義，請確保 template-loader.js 已載入');
}
