...
// 匯豐銀行模板引擎：根據配置應用模板提取數據行
// 注意：此函數包含匯豐銀行特定的邏輯（如 C/F BALANCE 停止）
function applyHSBCTemplate(lines, headerRow, config, pageWidth) {
    const dataRows = [];
    let foundEndMarker = false; // 標記是否找到 C/F BALANCE
    ...
    if (!headerXPositions) {
        ...
        return { dataRows, foundEndMarker };
    }
    ...
    // 匯豐銀行：從標題行開始提取數據（包含 C/F BALANCE 停止邏輯）
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        // 匯豐銀行：檢查是否為 B/F BALANCE 或 C/F BALANCE 行
        const isBFBalance = lineText.includes('B/F BALANCE') || lineText.includes('B/F BAL');
        const isCFBalance = lineText.includes('C/F BALANCE') || lineText.includes('C/F BAL');
        
        // 驗證規則：跳過明顯是標題或分隔線的行
        // 但不要跳過 B/F BALANCE 和 C/F BALANCE 行（這些是重要的數據行）
        if (extractionConfig.validation && !isBFBalance && !isCFBalance) {
            ...
            if (extractionConfig.validation.skipHeaderKeywords) {
                ...
                // 計算包含的關鍵字數量
                const matchedKeywords = headerKeywords.filter(keyword => lineTextLower.includes(keyword));
                const keywordCount = matchedKeywords.length;
                
                // 檢查是否有實際數據（日期或金額）
                const hasDate = line.some(item => isValidDate(item.text.trim()));
                const hasAmount = line.some(item => isAmount(item.text.trim()));
                const hasActualData = hasDate || hasAmount;
                
                // 只有同時包含多個標題關鍵字（至少2個）且沒有實際數據時，才會被跳過
                if (keywordCount >= 2 && !hasActualData) {
                    continue;
                }
            }
        ...
        if (extracted) {
            // 驗證提取的數據
            // B/F BALANCE 和 C/F BALANCE 行需要特殊處理，即使驗證失敗也要包含
            const shouldInclude = validateExtractedData(extracted, extractionConfig.validation) || 
                                 isBFBalance || isCFBalance;
            ...
            if (shouldInclude) {
                ...
                // 匯豐銀行：如果是 C/F BALANCE 行，提取後停止提取
                if (isCFBalance) {
                    foundEndMarker = true;
                    console.log(`✓ 找到 C/F BALANCE 標記，已提取該行，停止提取數據（行 ${lineIdx}）`);
                    break; // 停止循環
                }
            } else {
                if (lineIdx < 10) {
...
function extractRowDataByConfig(line, headerXPositions, extractionConfig, previousDate = '') {
    ...
    // 先檢查整行是否包含 B/F BALANCE 或 C/F BALANCE（用於特殊處理）
    const lineTextUpper = line.map(item => item.text).join(' ').toUpperCase();
    const isBFBalance = lineTextUpper.includes('B/F BALANCE') || lineTextUpper.includes('B/F BAL');
    const isCFBalance = lineTextUpper.includes('C/F BALANCE') || lineTextUpper.includes('C/F BAL');
    ...
    // 如果 Details 提取失敗，且是 B/F 或 C/F BALANCE 行，嘗試從整行中提取
    if ((!detailsResult || !detailsResult.text) && (isBFBalance || isCFBalance)) {
        // 在 Date 和 Deposit 之間查找包含 B/F 或 C/F BALANCE 的項目
        for (let i = 0; i < line.length; i++) {
            const item = line[i];
            const itemX = item.x;
            const itemText = item.text.trim();
            
            // 在 Date 結束到 Deposit 開始之間查找
            if (itemX >= headerXPositions.date.end && itemX < headerXPositions.deposit.start) {
                const itemTextUpper = itemText.toUpperCase();
                if (itemTextUpper.includes('B/F BALANCE') || itemTextUpper.includes('C/F BALANCE') ||
                    itemTextUpper.includes('B/F BAL') || itemTextUpper.includes('C/F BAL')) {
                    detailsResult = { text: itemText, index: i };
                    console.log(`  → 找到 B/F 或 C/F BALANCE Details: "${itemText}" (X=${itemX.toFixed(0)})`);
                    break;
                }
            }
        }
        
        // 如果還是沒找到，嘗試在整行中查找（放寬條件）
        if (!detailsResult || !detailsResult.text) {
            for (let i = 0; i < line.length; i++) {
                const item = line[i];
                const itemText = item.text.trim();
                ...
                if (itemTextUpper.includes('B/F BALANCE') || itemTextUpper.includes('C/F BALANCE') ||
                    itemTextUpper.includes('B/F BAL') || itemTextUpper.includes('C/F BAL')) {
                    detailsResult = { text: itemText, index: i };
                    console.log(`  → 在整行中找到 B/F 或 C/F BALANCE Details: "${itemText}"`);
                    break;
                }
            }
        }
    ...
    if (!detailsResult || !detailsResult.text) {
        ...
        // 如果仍然沒有找到 Details，對於 B/F 或 C/F BALANCE 行，使用整行文本作為 Details
        if (!detailsResult || !detailsResult.text) {
            if (isBFBalance || isCFBalance) {
                // 對於 B/F 或 C/F BALANCE 行，嘗試組合所有非日期、非金額的項目作為 Details
                const detailsParts = [];
                for (let i = 0; i < line.length; i++) {
                    const item = line[i];
                    const itemText = item.text.trim();
                    ...
                    // 跳過日期和金額
                    if (!isValidDate(itemText) && !isAmount(itemText) && itemText !== '') {
                        // 如果包含 B/F 或 C/F BALANCE，使用它
                        const itemTextUpper = itemText.toUpperCase();
                        if (itemTextUpper.includes('B/F') || itemTextUpper.includes('C/F')) {
                            detailsParts.push(itemText);
                        }
                    }
                }
                
                if (detailsParts.length > 0) {
                    detailsResult = { text: detailsParts.join(' '), index: 0 };
                    console.log(`  → B/F 或 C/F BALANCE 行，使用組合的 Details: "${detailsResult.text}"`);
                } else {
                    // 如果還是沒找到，使用 "B/F BALANCE" 或 "C/F BALANCE" 作為默認值
                    detailsResult = { 
                        text: isBFBalance ? 'B/F BALANCE' : 'C/F BALANCE', 
                        index: 0 
                    };
                    console.log(`  → B/F 或 C/F BALANCE 行，使用默認 Details: "${detailsResult.text}"`);
                }
            } else {
